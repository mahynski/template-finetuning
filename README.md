...
Then navigate to `projects/`, select the relevant project directory, and install the project into a virtual environment with uv.
...
Refer to local README.md files for additional instructions. For example, the `autotrain_ui` has an additional `setup.sh` script to run and needs you to specify a HuggingFace token before the UI will work correctly.
...
The logo for this repository (logo.png) was generated using ChatGPI on July 25, 2025 with the prompt "Create a logo that represents automatic finetuning of a foundation model."
